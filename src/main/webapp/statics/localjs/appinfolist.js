...
var saleSwitchAjax = function(appId,obj){
	$.ajax({
		type:"POST",//请求类型
		url:"http://127.0.0.1:8080/APP/jsonDyna/openSal.do",//请求的url
		contentType : "application/json;charset=UTF-8",
		data:appId,//请求参数
		dataType:"json",//ajax接口（请求url）返回的数据类型
		success:function(data){//data：返回数据（json对象）
...
$(".viewApp").on("click",function(){
	var obj = $(this);
	window.location.href="lookAppInFoView.do?appInfoid="+ obj.attr("appinfoid");
});
...
$(".deleteApp").on("click",function(){
	...
	if(confirm("你确定要删除APP应用【"+obj.attr("appsoftwarename")+"】及其所有的版本吗？")){
		$.ajax({
			type:"POST",
			url:"http://127.0.0.1:8080/APP/jsonDyna/deleteApp.do",
			data:obj.attr("appinfoid"),
			contentType : "application/json;charset=UTF-8",
			dataType:"json",
